...
void DrawTrack(const std::vector<Point2f>& point, const int index, const float scale, Mat& image, Scalar color, int unusual)
{
	...
	for (int j = 1; j <= index; j++) {
		...
		point1 *= scale;
                ...
                strength = cvFloor(255.0*(j+1.0)/float(index+1.0));

                if (unusual)
                {
                    line_color = Scalar(0, 0, strength);
                }
                else
                {
                    line_color = Scalar(0, strength, 0);
                };

		line(image, point0, point1, line_color, 2, 8, 0);
		point0 = point1;
	}
        circle(image, point0, 2, color, -1, 8, 0);
}
